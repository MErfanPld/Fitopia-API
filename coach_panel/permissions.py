...
def ensure_coach(user, gym_id=None):
    ...
    if coach:
        return coach
    access_qs = GymStaffAccess.objects.filter(user=user, is_active=True, role="coach")
    if gym_id is not None:
        access_qs = access_qs.filter(gym_id=gym_id)
    access = access_qs.select_related("gym").first()
    if not access:
        raise PermissionDenied("فقط مربی به پنل مربی دسترسی دارد.")
...
class IsCoachOnly(BasePermission):
    message = "دسترسی پنل مربی فقط برای مربی است."
    ...
    def has_permission(self, request, view):
        ...
        if GymCoach.objects.filter(user=request.user, is_active=True).exists():
            if gym_id is None:
                return True
            return (
                GymCoach.objects.filter(
                    user=request.user, gym_id=gym_id, is_active=True
                ).exists()
                or has_gym_access(request.user, gym_id)
            )
