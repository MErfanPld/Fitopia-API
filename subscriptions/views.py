...
class PurchaseSubscriptionView(views.APIView):
    ...
    @extend_schema(
        request=inline_serializer(
            ...
            fields={
                ...
                "use_discount": drf_serializers.BooleanField(
                    default=False, help_text="استفاده از تخفیف"
                ),
            },
        ),
    ...
    def post(self, request):
        ...
        use_discount = serializer.validated_data.get("use_discount", False)
        ...
        with transaction.atomic():
            ...
            discount_amount = 0
            discount_profile = None
            if use_discount:
                try:
                    discount_profile = (
                        UserDiscountProfile.objects.select_for_update()
                        .get(user=request.user)
                    )
                    if (
                        discount_profile.discount_amount > 0
                        and not discount_profile.is_used
                    ):
                        discount_amount = min(
                            discount_profile.discount_amount, plan.price
                        )
            ...
            if discount_amount > 0 and discount_profile is not None:
                discount_profile.use_discount()
...
class MyDiscountView(views.APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response(UserDiscountProfileSerializer(profile).data)
        except UserDiscountProfile.DoesNotExist:
            return Response(
                {
...
class ExpireSubscriptionView(views.APIView):
    ...
    def post(self, request, pk):
        ...
        subscription.expire()
        return Response(
            {"message": "اشتراک منقضی شد و توکن‌های باقیمانده ذخیره شدند."}
        )
...
class SubscriptionGymsAPIView(APIView):
    ...
    def get(self, request, pk):
        subscription = get_object_or_404(
            UserSubscription.objects.select_related("plan").prefetch_related(
                "plan__gyms"
            ),
            pk=pk,
        )
        ...
        return Response(SubscriptionGymsSerializer(subscription).data)
